...
Eventually, something is going to go wrong.  What is going to help you figure out what went wrong?  State management is also about mutating the state of the application, so you better be able to inspect the current state of the application easily.  In addition, you'll find it advantageous to "time travel" - basically, replay the state changes to see what went wrong.  I'm assuming that the developer has React DevTools installed (so using Edge, Chrome, or Firefox for debugging).
...
If you want to do your own weightings for the things that are important to you, check out the [Excel spreadsheet](/assets/images/2024/06/state-management-2024.xlsx) that I used for this post.
